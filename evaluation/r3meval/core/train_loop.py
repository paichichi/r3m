...
def bc_train_loop(job_data: dict) -> None:
    ...
    data_dir = '/home/jingfeng/xizhe/VLR_models/data/'
    ...
    while True:
        ...
        print("Step", last_step, flush=True)
        ...
        if ((agent.steps % job_data['eval_frequency']) < (last_step % job_data['eval_frequency'])):
            ...
            t0 = time.time()
            print(
                f"[EVAL] begin sampling: num_traj={job_data['eval_num_traj']} horizon={e.horizon} num_cpu={job_data['num_cpu']}",
                flush=True)

            paths = sample_paths(
                num_traj=job_data['eval_num_traj'],
                env=e,
                policy=agent.policy,
                eval_mode=True,
                horizon=e.horizon,
                base_seed=job_data['seed'] + epoch,
                num_cpu=job_data['num_cpu'],
                env_kwargs=env_kwargs,
                reset_fn=reset_fn
            )
            print(f"[EVAL] sampling done in {time.time() - t0:.1f}s, got {len(paths)} paths", flush=True)

            # 防止 images 太大拖慢后续 / 爆内存
            for p in paths:
                p.pop("images", None)

            t1 = time.time()
            print("[EVAL] begin evaluate_success", flush=True)
            try:
                success_percentage = e.env.unwrapped.evaluate_success(paths)
            except Exception:
                traceback.print_exc()
                raise
            print(f"[EVAL] evaluate_success done in {time.time() - t1:.1f}s, success={success_percentage}", flush=True)

            t2 = time.time()
            print("[EVAL] begin saving", flush=True)

            agent.logger.log_kv('eval_epoch', epoch)
            agent.logger.log_kv('eval_success', success_percentage)

            pickle.dump(agent.policy, open(f'./iterations/policy_{epoch}.pickle', 'wb'))
            agent.logger.save_log('./logs/')
            agent.logger.save_wb(step=agent.steps)

            print(f"[EVAL] saving done in {time.time() - t2:.1f}s", flush=True)

            print_data = sorted(
                filter(lambda v: np.asarray(v[1]).size == 1, agent.logger.get_current_log().items())
            )
            print(tabulate(print_data), flush=True)
        # ====== eval 定位块结束 ======
        epoch += 1
