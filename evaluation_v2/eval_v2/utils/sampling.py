...
from eval_v2.utils import tensor_utils
...
def aggregate_env_infos(env_infos_list):
    keys = set()
    for info in env_infos_list:
        keys.update(info.keys())

    out = {}
    T = len(env_infos_list)

    for k in keys:
        vals = [info.get(k, None) for info in env_infos_list]

        # 只要出现 dict/list/tuple/set，就强制做成 (T,) 的 object array
        if any(isinstance(v, (dict, list, tuple, set, np.ndarray)) for v in vals if v is not None):
            arr = np.empty((T,), dtype=object)
            arr[:] = vals
            out[k] = arr
        else:
            out[k] = np.array(vals)
...
def do_rollout(
    ...
    while ep < num_traj:
        ...
        path = dict(
            ...
            env_infos=aggregate_env_infos(env_infos),
            terminated=done,
